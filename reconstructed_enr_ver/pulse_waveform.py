...
class pulse_lib:
    ...
    def cos_waveform(self, t, args):
        ...
        if 0 < t_relative < self.t_rising:
            return self.ampli/2 * np.cos(self.freq * np.pi * 2 * t_relative + self.phase)* (1 - np.cos(np.pi * t_relative / self.t_rising))
        ...
        if self.t_g - self.t_rising < t_relative  < self.t_g:
            return self.ampli/2 * np.cos(self.freq * np.pi * 2 * t_relative + self.phase)* (1 - np.cos(np.pi * ( t_relative - self.t_g + 2 * self.t_rising)/self.t_rising))
