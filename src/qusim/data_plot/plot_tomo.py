...
# qutip 5 migration: the qutip-4 internal module paths (qutip.qobj,
# qutip.superoperator, qutip.superop_reps) were removed. Qobj,
# vector_to_operator and settings are still top-level in qutip 5. The two
# superoperator helpers survive under qutip.core.superop_reps: the qubit-dims
# check is now the public ``isqubitdims`` and ``_super_to_superpauli`` was
# renamed ``_to_superpauli`` (same behavior). We re-alias them to the original
# names so the code below is unchanged.
