...
from qusim.pulse_gen.simulation_option import SimulationOption
from qusim.pulse_gen.pulse_buffer import merge_pulse_chan
import qusim.instruments.tools as tool
from qusim.instruments.angle import get_angle
from qusim.pulse_gen.noise_config import *
from qusim.pulse_gen.noise_gen import noise_gen

# qutip 5: solver options are a plain dict (qutip.Options is a deprecated shim).
DEFAULT_SOLVER_OPTIONS = {'rtol': 1e-8}


def hermitian_conjugate(matrix):
    # Convert the input list to a NumPy array
    # numpy 2.0 removed the `np.complex_` alias -> use the explicit dtype.
    np_matrix = np.array(matrix, dtype=np.complex128)
...
class ArbQubitSys:
    ...
    """
    def __init__(self, freq_list, inter_list=None, r=0, gamma_list=None, driving_list = None, bias_list = None, extra_list = None):
    ...
    def get_state_index(self, n:tuple, freq_threshold = 1e-6, deg_threshold = 5e-3, deg_round = 7):
        ...
        # qutip 5: Qobj indexing drops one nesting level (arr[i] -> 1-elem array,
        # arr[i][0] -> scalar), so the old `[0][0]` over-indexes. Keep scalar intent.
            ...
            for index, value in enumerate(eigen_list):
                ...
                if np.abs(value - max_value) < deg_threshold: # Not sufficient to say degeneracy is appears
                    ...
                    for row in self.H.eigenstates()[1][index]:
                        # qutip 5: iterating a ket yields 1-elem arrays; row[0] is the scalar.
    ...
    def master_eq_solver(self, H_d, tlist: np.ndarray, initial_state: List[Qobj], option = None):
        options = option if option is not None else DEFAULT_SOLVER_OPTIONS
        # `self.co_list` is a @property returning the collapse-operator list (see below),
        # so it is passed WITHOUT `()`. The old positional `[]` e_ops slot is dropped
        # (empty e_ops is the qutip-5 default, and e_ops is now keyword-only).
        result = mesolve(H_d, initial_state, tlist, self.co_list, options = options)
    ...
    def system_dynamics_propagator(self, 
        ...
        H_d = self.get_h_d(pseq, sim_opts, channel_noise)
        options = option if option is not None else DEFAULT_SOLVER_OPTIONS
        # qutip 5 `propagator` no longer accepts `parallel=`/`progress_bar=` kwargs;
        # progress is requested through the options dict instead.
        if do_progress_bar:
            options = dict(options)
            options['progress_bar'] = do_progress_bar if isinstance(do_progress_bar, str) else 'tqdm'
        # NOTE: `do_parallel` is kept for backward-compatible API but is a no-op under
        # qutip 5 -- `propagator` has no parallelism knob in its public signature.
        # `self.co_list` is a @property returning the collapse-operator list, so no `()`.
        # qutip 5: array-based time dependence needs the sample grid passed via `tlist=`
        # (the positional `t` only selects output times).
    ...
    def get_data_list(self, result_list, sim_opts: SimulationOption, state_list):
        ...
        for state in state_list:
            ...
            for ii in range(0, sim_opts.simulation_point):
                # qutip 5: bra * ket returns a Python complex scalar (was a 1x1 Qobj in
                # qutip 4), so the old `[0][0][0]` indexing raises TypeError. Drop it.
                data_list_dummy.append(np.abs((result_list.states[ii]).dag()*state)**2)
