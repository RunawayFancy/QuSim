...
class TransmonSys:
    ...
    def __init__(self, N, q_dim, w, alpha, r=0, gamma_list = None, g_freq = False):
        self.w = w
    ...
    def get_state_index(self, n, freq_threshold = 1e-6, deg_threshold = 5e-3, deg_round = 7):
        ...
        eigen_list = [np.abs(arr[state_index]) for arr in self.H.eigenstates()[1]]
    ...
    def system_dynamics_mesolve(self, pseq: List[PulseConfig], sim_opts: SimulationOption, option = None):
        """
    ...
    def master_eq_solver(self, H_d, tlist: np.ndarray, initial_state: List[Qobj], option = None):
        ...
        result = mesolve(H_d, initial_state, tlist, c_ops = self.co_list(), options = options)
        angle = get_angle(initial_state, result)

        return result, angle

    def system_dynamics_propagator(self, pseq: List[PulseConfig], sim_opts: SimulationOption, option = None):
        H_d = []; pulse_buffer_list = [[] for _ in range(3)]
        H_d.append(self.H)
        for pulse in pseq:
            pulse_buffer_list = merge_pulse_chan(pulse_buffer_list, pulse, self.send_pulse(pulse, sim_opts))
        for Hd_i in pulse_buffer_list[2]:
            H_d.append(Hd_i)
        options = option if option is not None else DEFAULT_SOLVER_OPTIONS
        # qutip 5: for array-based time dependence ([H0, [H1, coeff_array], ...]) the
        # sample grid must be given to `propagator` via the `tlist=` keyword; the
        # positional `t` only selects the output times. See qutip propagator docs.
