...
class NoiseTimeConfig:
    ...
    time_dependent: bool = False,
        ...
        False: ~ will be time-independent
            It does not require trafofn.

    trafofn: Optional[Callable[[np.ndarray, np.ndarray], np.ndarray]] = None
        ...
        trafofn input
            trafofn(tlist: np.ndarray, waveform, np.ndarray) -> np.ndarray
    ...
    def __init__(self,
        ...
        time_dependent: bool = False,
        trafofn: Optional[Callable] = None,
        ...
        self.trafofn = trafofn

        if tdbasefn is not None:
            # Ensure trafofn is callable
            if not callable(tdbasefn):
                raise ValueError("trafofn should be a callable function or None.")
            
            # Check the signature of trafofn
